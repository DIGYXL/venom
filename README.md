# 🕷️ venom - A Simple Red Team Framework
...
4. If you're on Windows, run the downloaded `.exe` file.
5. If you're on macOS or Linux, extract the downloaded compressed file and open a terminal to run the application.
...
2. **Navigate to the Venom Directory:**
   Use the `cd` command to change directories to where you installed Venom. For example:
   ```bash
   cd path/to/venom
   ```
