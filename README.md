...
Venom C2 is a dependency‑free Python3 Command & Control framework for redteam persistence, built to run on production Linux systems without installing packages. It comprises a Flask team server, an Electron operator GUI, and a single‑file Python agent that communicates over HTTP/HTTPS using configurable AES‑encrypted JSON messages. Use it to execute commands, manage files, maintain access, and create SSH reverse tunnels from compromised systems.
...
```bash
git clone <your-venom-repo-url> venom
cd venom
```
